	...
	public DogsList(PageParameters pageParams) {
		...
		for (String dogProperty : DogsDataProvider.DOG_PROPERTIES) {
			...
			if (DogsDataProvider.IMAGE_PROPERTY.equals(dogProperty)) {
				Json.RawValue renderFunction = new Json.RawValue("function(data, type, row, meta) {" + "var html = '<div class=\"row\">'; "
						+ " $.each(data, function(index,value) { html += '<div class=\"thumbnail col-xs-4\"><a id=\"a_' + value + '\" href=\"#\" ><img src=\"" + snapshotUrl + "?imageId=' + value + '\"/></a></div>';"
						+ " }); "
						+ " html += '</div>'; " 
						+ "	 return html;}");
